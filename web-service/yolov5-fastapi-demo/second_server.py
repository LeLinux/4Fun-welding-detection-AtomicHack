...
app = FastAPI()
app.mount("/templates/static", StaticFiles(directory=Path(__file__).parent.absolute() / "templates/static"), name="static",)
templates = Jinja2Templates(directory = 'web-service/yolov5-fastapi-demo/templates')
...
def detect_with_server_side_rendering(request: Request,
    ...
    model = torch.hub.load('yolov5', 'custom', path='models/best_1.pt', source='local') 
